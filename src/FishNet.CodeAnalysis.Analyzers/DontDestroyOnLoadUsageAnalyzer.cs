...
internal sealed class DontDestroyOnLoadUsageAnalyzer : DiagnosticAnalyzer
{
	...
	private void Analyze(SyntaxNodeAnalysisContext analysisContext)
	{
		...
		{
			foreach (ITypeSymbol supertype in analysisContext.SemanticModel.GetTypeSymbol(argument.Expression).EnumerateTypeHierarchy())
			{
				...
				analysisContext.ReportDiagnostic(Diagnostic.Create(Descriptor1, Location.Create(analysisContext.Node.SyntaxTree, argument.Span), supertype.Name));
